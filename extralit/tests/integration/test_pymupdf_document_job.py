...
async def test_document_upload_job():
    ...
    try:
        ...
        preprocessing_result = preprocessing.preprocessor.preprocess(file_data=pdf_bytes, filename=test_pdf.name)
        ...
        print(f"❌ Error during document upload job test: {e!s}")
        import traceback
