...
# Try to import pandera and extralit, but make them optional
try:
...
class SchemaService:
    ...
    async def get_workspace_schema_configuration(
        ...
        return workspace.metadata_.get("schema_configuration", {
            "schemas": [], 
            "last_sync": None
        })
    ...
    async def sync_s3_schema_to_metadata(
        ...
        try:
            ...
            schema = from_json(BytesIO(json.dumps(schema_data).encode('utf-8')))
    ...
    async def create_schema_structure_from_metadata(
        self,
        workspace: Workspace
    ) -> Optional["SchemaStructure"]:
        """
        ...
        for schema_data in schemas_data:
            try:
                ...
                file_response = files.get_object(
                    ...
                    bucket_name,
                    schema_data["s3_path"],
                    version_id=schema_data.get("version_id")
                )
        ...
        schema_structure = SchemaStructure(
            schemas=schemas,
            singleton_schema=singleton_schema
        )
