...
def storage(monkeypatch, tmp_path) -> ObjectStorage:
    monkeypatch.setattr(files.settings, "s3_endpoint", None)
    monkeypatch.setattr(files.settings, "home_path", str(tmp_path))
...
class TestRoundtrip:
    async def test_put_then_get_returns_the_body(self, storage):
        await files.put_object(storage, BUCKET, "a/b.txt", b"hello world", content_type="text/plain")

        file = await files.get_object(storage, BUCKET, "a/b.txt")
    ...
    async def test_put_accepts_str_and_encodes_utf8(self, storage):
        await files.put_object(storage, BUCKET, "s.json", '{"k": "é"}', content_type="application/json")

        file = await files.get_object(storage, BUCKET, "s.json")
    ...
    async def test_a_key_containing_dots_roundtrips(self, storage):
        ...
        await files.put_object(storage, BUCKET, "pdf/a.b.pdf", b"%PDF-1.4")

        file = await files.get_object(storage, BUCKET, "pdf/a.b.pdf")
    ...
    async def test_put_overwrites_in_place(self, storage):
        await files.put_object(storage, BUCKET, "k.txt", b"first")
        await files.put_object(storage, BUCKET, "k.txt", b"second")

        file = await files.get_object(storage, BUCKET, "k.txt")
...
class TestRanges:
    async def test_a_single_byte_range_returns_one_byte(self, storage):
        ...
        await files.put_object(storage, BUCKET, "k.bin", b"0123456789")
...
class TestListing:
    async def test_list_is_recursive_by_default(self, storage):
        for key in ("p/one.txt", "p/nested/two.txt", "other.txt"):
            await files.put_object(storage, BUCKET, key, b"x")

        listing = await files.list_objects(storage, BUCKET, prefix="p")
    ...
    async def test_non_recursive_list_stops_at_the_delimiter(self, storage):
        ...
        listing = await files.list_objects(storage, BUCKET, prefix="p", recursive=False)
    ...
    async def test_listing_an_empty_prefix_is_not_an_error(self, storage):
        await files.put_object(storage, BUCKET, "k.txt", b"x")

        listing = await files.list_objects(storage, BUCKET, prefix="absent")
...
class TestDeletion:
    async def test_delete_then_get_raises_404(self, storage):
        await files.put_object(storage, BUCKET, "k.txt", b"x")
        await files.delete_object(storage, BUCKET, "k.txt")

        with pytest.raises(HTTPException) as error:
            await files.get_object(storage, BUCKET, "k.txt")

        assert error.value.status_code == 404

    async def test_deleting_an_absent_key_is_a_no_op(self, storage):
        await files.delete_object(storage, BUCKET, "never/existed.txt")
    ...
    async def test_a_deleted_object_leaves_the_listing(self, storage):
        ...
        await files.put_object(storage, BUCKET, "p/k.txt", b"x")
        await files.delete_object(storage, BUCKET, "p/k.txt")
...
class TestContentType:
    async def test_it_is_derived_from_the_key_extension(self, storage):
        await files.put_object(storage, BUCKET, "a/b.txt", b"x", content_type="text/plain")
    ...
    async def test_extensionless_minted_keys_fall_back_to_their_prefix(self, storage):
        ...
        await files.put_object(storage, BUCKET, "pdf/some-uuid", b"%PDF-1.4", content_type="application/pdf")

        file = await files.get_object(storage, BUCKET, "pdf/some-uuid")

        assert file.metadata.content_type == "application/pdf"

    async def test_an_unknown_key_defaults_to_octet_stream(self, storage):
        await files.put_object(storage, BUCKET, "mystery", b"x")

        file = await files.get_object(storage, BUCKET, "mystery")
...
class TestDocumentUrls:
    async def test_download_file_content_reads_the_whole_body(self, storage):
        await files.put_object(storage, BUCKET, "pdf/doc-1", b"%PDF-1.4 body")
        url = files.get_proxy_document_url(BUCKET, "pdf/doc-1")
...
class TestPutDocumentFile:
    async def test_the_first_upload_returns_a_proxy_url(self, storage):
        ...
        url = await files.put_document_file(storage, BUCKET, uuid4(), b"%PDF-1.4", "a.pdf")

        assert url is not None
        assert url.startswith(f"/api/v1/file/{BUCKET}/pdf/")
