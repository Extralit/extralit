...
class TestColumnFieldSettingsAreNotPatchable:
    def test_the_update_union_has_no_column_member(self):
        # A column field is derived from the dataset's Pandera schema version, so
        # `PATCH /fields/{id}` must not be a second, unvalidated way to change its dtype —
        # that would contradict the immutability
        # `contexts/schema_versions._reject_incompatible_columns` enforces at publish time.
        # Republishing the schema version is the only way to change a column.
        with pytest.raises(ValidationError, match="union_tag_invalid"):
            TypeAdapter(FieldSettingsUpdate).validate_python({"type": "column", "dtype": "int64"})
