...
def preprocess(df: pd.DataFrame, required_columns: List[str] = [], drop_columns: Optional[List[str]] = None):
    """
...
def df_to_json(
    ...
    **kwargs,
    ...
    try:
        df_json = json.loads(df.to_json(orient="table", index=bool(df.index.name) or len(df.index.names) > 1))
    except Exception as e:
...
def schema_to_json(dataframe_schema: pa.DataFrameSchema) -> Dict[str, Any]:
    ...
    for check in schema_specs["checks"] or []:
        ...
        schema_specs["checks"][check] = {
            k: v
            for k, v in zip(
                schema_specs["checks"][check].keys(), make_same_length_arguments(**schema_specs["checks"][check])
            )
        }
...
def json_to_df(
    input: Union[str, List[Dict[str, Any]], Dict[str, Dict[str, Any]], ExtractionResponse],
...
def parse_json_to_df(input: str) -> pd.DataFrame:
    try:
        df = pd.read_json(io.StringIO(input) if isinstance(input, str) else input, orient="table", convert_dates=False)
    except:
...
def convert_to_schema_dtypes(
    df: pd.DataFrame, schema: pa.DataFrameSchema, errors: Literal["raise", "ignore"] = "ignore"
) -> pd.DataFrame:
    dtype_map = {"int64": "Int64", "int": "Int64", "int32": "Int64"}

    data_types = {
        col: dtype_map.get(str(datatype), str(datatype)) for col, datatype in schema.dtypes.items() if col in df.columns
    }
