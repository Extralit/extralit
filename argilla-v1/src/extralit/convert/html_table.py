...
def html_table_to_json(s: str) -> str:
    ...
    df_json = df.to_json(orient="table", index=bool(df.index.name) or len(df.index.names) > 1)
    return df_json
...
def html_to_df(
    ...
    convert_spanning_rows=False,
    remove_markdown=False,
    rename_duplicate_columns=True,
) -> pd.DataFrame:
    if not isinstance(s, io.StringIO):
    ...
    if isinstance(df.columns, pd.MultiIndex):
        new_columns = [tuple(re.sub(r"\.\d+$", "", level) for level in levels) for levels in df.columns]
        df.columns = pd.MultiIndex.from_tuples(new_columns)

    df.columns = df.columns.map(
        lambda column: re.sub(r"Unnamed: \d+", "Variable", column) if isinstance(column, str) else column
    )
...
def flatten_multilevel_columns(columns: Union[pd.MultiIndex, pd.Index], sep=" - ") -> pd.Index:
    if isinstance(columns, pd.MultiIndex):
        new_columns = columns.map(
            lambda levels: sep.join(
                list(dict.fromkeys(str(level) for level in levels if level))
