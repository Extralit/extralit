...
class Alignments(BaseModel):
    ...
    def insert_missing_tables(self, records: List[rg.FeedbackRecord], **kwargs):
        ...
        for number in range(1, max_number + 1):
            ...
            if number > 1 and numbers_count.get(number - 1, 1e6) > 1:
                continue
...
def merge_extractions(**extraction_sources: Dict[str, Segments]) -> Alignments:
    extraction_sources = {
        source: segments for source, segments in extraction_sources.items() if segments is not None and len(segments)
    }
    pointers = {source: 0 for source in extraction_sources}
    merged_data = []

    while any(
        pointer < len(extraction_list)
        for pointer, extraction_list in zip(pointers.values(), extraction_sources.values())
    ):
        current_items = []
        ...
        filtered_items: List[Tuple[str, TextSegment]] = [
            (source, segment) for source, segment, *number in valid_items if segment.page_number == current_page_number
        ]
        ...
        summary = next(
            (segment.summary for source, segment in filtered_items if getattr(segment, "summary", None)), None
        )
        number = next((segment.number for source, segment in filtered_items if getattr(segment, "number", None)), None)
        image = next((segment.image for source, segment in filtered_items if getattr(segment, "image", None)), None)
        probabilities = [
            segment.probability for source, segment in filtered_items if getattr(segment, "probability", None)
        ]
...
def find_matching_text_elem(
    text: str, elements: List[Element], page_number: int, start_index: Optional[int] = None, thresh=0.5
) -> List[int]:
    matches = []
