        ...
        for line in page.split("\n"):
            ...
            if header_match:
                ...
                current_segment = TextSegment(
                    header=f"{stored_header}{header_match.group(2)}", level=level, page_number=page_number, text=""
                )

                if parent:
                    current_segment.relationships[NodeRelationship.PARENT] = RelatedNodeInfo(
                        node_id=parent.id,
                    )

                    parent.relationships.setdefault(NodeRelationship.CHILD, []).append(
                        RelatedNodeInfo(
                            node_id=current_segment.id,
                        )
                    )
                stored_header = ""
