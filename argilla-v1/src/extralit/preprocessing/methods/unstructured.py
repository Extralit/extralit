...
def get_table_segments(elements: List[Element], max_caption_look_head=5, output_dir=None, redo=False) -> Segments:
    ...
    for i, elem in enumerate(elements):
        ...
        header, footer = get_table_header_footer(
            ...
            footer_pattern=r".*",
            header_filter_fn=lambda x: isinstance(x, (Text, FigureCaption))
            and page_number == x.metadata.page_number
            and coordinates.is_vstacked(Coordinates(**x.metadata.coordinates.to_dict()), width="smaller") != False,
            footer_filter_fn=lambda x: isinstance(x, (Text, FigureCaption))
            and page_number == x.metadata.page_number
            and coordinates.is_vstacked(Coordinates(**x.metadata.coordinates.to_dict()), width="smaller") != False,
...
def get_figure_segments(elements: List[Element], skip_empty_header=True, max_caption_look_head=5) -> Segments:
    figures = Segments()
    captions_taken = set()
    for i, elem in enumerate(elements):
