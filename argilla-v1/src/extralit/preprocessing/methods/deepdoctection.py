    ...
    for page_num, page in enumerate(pages, start=1):
        ...
        for table_num, table in enumerate(page.tables, start=1):
            ...
            image_path = extract_image(
                page_image, coordinates, title=f"table_{page_num}_{table_num}.png", output_dir=output_dir, redo=redo
            )

            table_index = next(
                (i for i, ann in enumerate(page.annotations) if ann._annotation_id == table.annotation_id), None
            )
            ...
            header, footer = get_table_header_footer(
                ...
                footer_pattern=r".*",
                header_filter_fn=lambda x: x._category_name == LayoutType.text
                and coordinates.is_vstacked(get_coordinates(x, page), width="smaller") != False,
                footer_filter_fn=lambda x: x._category_name == LayoutType.text
                and coordinates.is_vstacked(get_coordinates(x, page), width="smaller") != False,
...
def get_coordinates(layout: ImageAnnotationBaseView, page: Page):
    ...
    coordinates = Coordinates(
        points=[[bbox.ulx, bbox.uly], [bbox.lrx, bbox.uly], [bbox.ulx, bbox.lry], [bbox.lrx, bbox.lry]],
