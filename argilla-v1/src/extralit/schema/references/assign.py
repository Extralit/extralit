...
def get_unique_index(df: pd.DataFrame, group_columns: List[str], prefix="", suffix="", n_digits=2) -> pd.DataFrame:
...
def map_items_to_references(
    items: Union[pd.Series, pd.DataFrame], reference_items: pd.DataFrame, threshold: float = 80
) -> List[str]:
    """
