...
def register_check_methods() -> None:
    ...
    if check_less_than.__name__ not in pa.Check:
        register_check_method(
            statistics=["columns_a", "columns_b", "or_equal"], supported_types=pd.DataFrame, check_type="vectorized"
        )(check_less_than)
    ...
    if check_greater_than.__name__ not in pa.Check:
        register_check_method(statistics=["col_a", "col_b", "or_equal"])(check_greater_than)
    ...
    if singleton.__name__ not in pa.Check:
        register_check_method(statistics=["enabled"], check_type="vectorized")(singleton)
