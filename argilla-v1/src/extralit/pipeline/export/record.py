...
def create_extraction_records(
    ...
    """
    assert (
        isinstance(dataset, RemoteFeedbackDataset) or dataset is None
    ...
    for ref, extractions in paper_extractions.items():
        ...
        if dataset is not None:
            if isinstance(paper.file_path, str):
                doc = dataset.add_document(
                    rg.Document.from_file(
                        paper.file_path, reference=ref, pmid=paper.get("pmid"), doi=paper.get("doi"), id=paper.get("id")
                    )
                )
            else:
        ...
        for schema_name, extraction in extractions.items():
            ...
            if responses and ref in responses and schema_name in responses[ref].items:
                ...
                nodes_df.drop(
                    columns=nodes_df.columns.difference(["relevance", "header", "page_number", "text"]),
                    errors="ignore",
                    inplace=True,
                )
                # nodes_df['page_number'] = nodes_df['page_number'].map(lambda x: f"[Page {x}](#page_number.{x})" if x else None)
                fields["context"] = nodes_df.style.background_gradient(
                    axis=1, subset=["relevance"], cmap="RdYlGn"
...
def create_publication_records(
    ...
    """
    assert (
        papers.index.name == "reference"
    ), f"The given dataframe must have index name as 'reference', given {papers.index.name}"
    records = []
    ...
    for reference, paper in tqdm(papers.iterrows()):
        ...
        publication_metadata = {k: v for k, v in paper.to_dict().items() if k in dataset_field_names}
        metadata.update(publication_metadata)
        publication_metadata = pd.Series(publication_metadata, name=paper.name)

        fields = {
            **{k: v for k, v in paper.to_dict().items() if k in dataset_field_names and not pd.isna(v)},
        }
...
def generate_empty_extraction(schema: pa.DataFrameSchema, size=2) -> pd.DataFrame:
    ...
    df = pd.DataFrame.from_dict({col: default_value for i, col in enumerate(schema.columns) if i < 5})
    ...
    if isinstance(schema.index, pa.MultiIndex):
        ...
        index = pd.MultiIndex.from_tuples(
            [tuple(f"{prefix}{i+1}" for prefix in index_prefixes) for i in range(size)], names=index_names
        )
    elif schema.index:
