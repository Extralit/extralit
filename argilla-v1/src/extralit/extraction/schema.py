...
def pandera_column_to_pydantic_field(column: pa.Column, validate_assignment=False, description_only=False) -> Field:
    ...
    if not validate_assignment:
        return Field(
            None, title=column.title, description=description, **(dict(json_schema_extra=extra) if extra else {})
        )
...
def get_extraction_schema_model(
...
) -> Type[SchemaStructuredOutputResponseModel]:
    """
    ...
    if not singleton:
        ...
        top_level_model = create_model(
            ...
            items=(List[lower_level_model], Field(default_factory=list)),
        )

    else:
        top_level_model = create_model(
            __model_name=top_class, __base__=SchemaStructuredOutputResponseModel, **indexes, **columns
        )
