...
def create_nodes(
    ...
    **nougat_kwargs,
    ...
    if preprocessing_dataset is not None:
        ...
        text_segments = (
            Segments.parse_raw(file_handler.read_text(texts_path)) if file_handler.exists(texts_path) else Segments()
        )
        table_segments = (
            Segments.parse_raw(file_handler.read_text(tables_path)) if file_handler.exists(tables_path) else Segments()
        )
...
def create_text_nodes(
    text_segments: Segments, extra_metadata: Optional[Dict[str, Any]], exclude_llm_metadata_keys: Iterable
) -> List[Document]:
    text_documents = []
    for i, segment in enumerate(text_segments.items):
        ...
        if "references" in segment.header.lower():
            ...
            len(segment.text) < 1000
            and "conflicts of interest" in segment.header.lower()
            or "acknowledgements" in segment.header.lower()
            or re.search(r"author.*contributions", segment.header.lower())
        ):
            continue
