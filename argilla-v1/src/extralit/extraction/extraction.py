...
def extract_schema(
    ...
    **kwargs,
    ...
    if types:
        filters.filters.append(MetadataFilter(key="type", value=types, operator=FilterOperator.IN))
    ...
    if isinstance(system_prompt, TextPromptClient):
        ...
        _LOGGER.warning(
            f"Invalid system_prompt type: {type(system_prompt)}, reverting to " f"DATA_EXTRACTION_SYSTEM_PROMPT_TMPL."
        )
...
def extract_paper(
    ...
    assert (
        index.service_context.llm.model == llm_models[0]
    ...
    for schema_name in extractions.schemas.ordering:
        ...
        df = extract_schema_with_fallback(
            schema=schema, extractions=extractions, index=index, responses=responses, models=llm_models, verbose=verbose
        )
...
def extract_schema_with_fallback(
    schema: pa.DataFrameSchema,
    extractions: PaperExtraction,
    index: VectorStoreIndex,
