...
def create_local_index(
    ...
    if global_handler and hasattr(global_handler, "set_trace_params"):
        global_handler.set_trace_params(name=f"embed-{paper.name}", tags=[paper.name])
...
def load_index(
    ...
    **kwargs,
    ...
    storage_context = get_storage_context(
        weaviate_client=weaviate_client, index_name=index_name, persist_dir=join(persist_dir, paper.name, embed_model)
    )
    ...
    if not isinstance(storage_context.vector_store, SimpleVectorStore):
        index = VectorStoreIndex.from_vector_store(storage_context.vector_store, service_context=service_context)
    else:
...
def load_index_retriever(
    ...
    **kwargs,
    ...
    llm = OpenAIEmbedding(model=embed_model, mode=retrieval_mode)
