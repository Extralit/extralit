...
def grits_from_pandas(
    ...
    to_html_args = dict(index=False, na_rep="", float_format=lambda x: "%.0f" % x if x == round(x) else "%.2f" % x)
    ...
    if reduce == "table":
        ...
        for col in true_df.columns.difference(exclude_columns or []):
            ...
            outputs[col] = convert_metrics_to_df(
                grits_from_html(true_html, pred_html, metrics=metrics), format="series"
            )
...
def grits_multi_tables(
    true_tables: List[Union[pd.DataFrame, str]],
    pred_tables: List[Union[pd.DataFrame, str]],
        ...
        try:
            if isinstance(true_df, pd.DataFrame):
                if only_common_columns:
                    only_columns = (
                        true_df.columns.intersection(pred_df.columns)
                        .difference(kwargs.get("index_columns", []))
                        .tolist()
                    )
                ...
                results[i] = grits_from_pandas(true_df, pred_df, format="series", only_columns=only_columns, **kwargs)
