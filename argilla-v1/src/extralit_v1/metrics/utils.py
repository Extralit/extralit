...
def harmonize_columns(true_df: pd.DataFrame, pred_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    pred_df = pred_df.astype(true_df.dtypes.drop(index=true_df.columns.difference(pred_df.columns)), errors="ignore")
...
def convert_metrics_to_df(
    metrics: Dict[str, float], format: Literal["series", "dataframe"] = "series", n_levels=2
