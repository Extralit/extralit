...
def check_time_difference(
    ...
    *,
    ...
    start_dates = pd.to_datetime(
        df[start_year].astype(str) + "-" + df[start_month].astype(str), format="%Y-%m", yearfirst=True, errors="coerce"
    )
    end_dates = pd.to_datetime(
        df[end_year].astype(str) + "-" + df[end_month].astype(str), format="%Y-%m", yearfirst=True, errors="coerce"
    )
