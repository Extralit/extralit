...
def create_papers_dataset(
    ...
    **kwargs,
    ...
    questions = []
    metadata_properties = {}
    assert not metadata_columns or papers.columns.intersection(metadata_columns).size == len(
        metadata_columns
    ), "Some column in `metadata_columns` not found in the papers dataframe"
    assert not span_columns or papers.columns.intersection(span_columns).size == len(
        span_columns
    ...
    for index_name in schema.index.names if schema.index else []:
        metadata_properties[index_name] = rg.TermsMetadataProperty(
            name=index_name, title=index_name.capitalize(), visible_for_annotators=True
        )
        ...
        try:
            if dtype == bool:
                metadata_prop = rg.TermsMetadataProperty(
                    name=column_name, title=column_name.capitalize(), visible_for_annotators=True
                )
            elif dtype == float:
...
def create_extraction_dataset(
    ...
    vectors_settings: Optional[List[rg.VectorSettings]] = None,
) -> rg.FeedbackDataset:
    extraction_dataset = rg.FeedbackDataset(
        ...
        metadata_properties=[
            ...
            rg.TermsMetadataProperty(name="type", title="Question Type", visible_for_annotators=True),
...
def create_preprocessing_dataset():
    dataset = rg.FeedbackDataset(
        fields=[
            rg.TextField(name="metadata", title="Metadata:", required=True, use_markdown=True),
            rg.TextField(name="header", title="Title:", required=True, use_markdown=True),
            rg.TextField(name="image", title="Image:", required=False, use_markdown=True),
            rg.TextField(name="text-1", title="Method 1:", required=False, use_markdown=True, use_table=False),
            rg.TextField(name="text-2", title="Method 2:", required=False, use_markdown=True, use_table=False),
            rg.TextField(name="text-3", title="Method 3:", required=False, use_markdown=True, use_table=False),
            rg.TextField(name="text-4", title="Method 4:", required=False, use_markdown=True, use_table=False),
            rg.TextField(name="text-5", title="Method 5:", required=False, use_markdown=True, use_table=False),
        ],
        questions=[
            rg.LabelQuestion(
                name="ranking",
