...
def get_paper_extraction_status(
    ...
    paper_records: List[RemoteFeedbackRecord] = paper_dataset.filter_by(
        metadata_filters=rg.TermsMetadataFilter(name="reference", values=references)
    ).records
    ...
    for record in paper_records:
        ...
        values = get_record_data(
            record,
            answers=document_schema.columns,
        ...
        user_statuses = {
            users_id_to_username.get(response.user_id, "NA"): response.status.name for response in record.responses
        }
...
def get_paper_extractions(
    ...
    records: List[RemoteFeedbackRecord] = dataset.filter_by(
        metadata_filters=rg.TermsMetadataFilter(name="reference", values=[reference])
    ).records
