...
def get_paper_tables(
    ...
    response_status: List[Literal["discarded", "submitted", "pending", "draft"]] = ["submitted"],
) -> Segments:
    """
    ...
    records: List[FeedbackRecord] = dataset.filter_by(
        metadata_filters=rg.TermsMetadataFilter(name="reference", values=[paper.name]), response_status=response_status
    ).records
    ...
    for record in records:
        values = get_record_data(
            ...
            answers=["text-correction", "header-correction", "footer-correction", "ranking", "duration"],
