...
def get_langfuse_traces(
    ...
    while max_page is None or page <= max_page:
        ...
        for trace in traces_batch.data:
            if trace is None or not trace.metadata:
                continue
            trace_metadata: dict = trace.metadata.get("metadata", {}) or {}
            if references and not next(
                (metadata.get("reference") in references for metadata in trace_metadata.values()), None
            ):
                continue
            if input_match and not re.search(input_match, trace.input):
                continue
            # if schema_names and not next((metadata.get('schema') in schema_names for metadata in trace.metadata.values()), None):
            #     continue
            if not isinstance(trace.output, dict) or not trace.output.get("items", []):
                continue
