...
class MonitorIntegrationDataset:
    ...
    def monitor(self, batch_size: int = 5, infinite: bool = True) -> None:
        ...
        while True:
            for record in self.from_dataset:
                ...
                if (
                    "updated_at" not in extractions_batch[reference]
                    or not extractions_batch[reference]["updated_at"]
                    or (updated_at and updated_at > extractions_batch[reference]["updated_at"])
                ):
    ...
    def process_batch(self, extractions_batch: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Optional[str]]]:
        ...
        if update_records:
            self._logger.info(
                f"Updating {len(update_records)} records: \n"
        ...
        if add_records:
            self._logger.info(
                f"Adding {len(add_records)} records: \n"
