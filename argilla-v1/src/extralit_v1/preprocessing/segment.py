    ...
    def from_pdffigures2(cls, json_file: str) -> "Segments":
        ...
        for item in data:
            mapped_item = {
                ...
                "coordinates": {
                    "points": [
                        [item["regionBoundary"]["x1"], item["regionBoundary"]["y1"]],
        ...
        items = sorted(items, key=lambda x: (x.page_number, x.number or float("inf")))
...
class TableSegment(TextSegment):
    ...
    def to_json(self) -> str:
        ...
        try:
            if self.source == "llmsherpa":
                df = llmsherpa_html_to_df(self.html)
                json = df.to_json(orient="table", index=bool(df.index.name) or len(df.index.names) > 1)
