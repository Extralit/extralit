...
def get_table_segments(document: Document, caption_pattern=r"(?i)(Table)\s?(\d+\.?)(.*|$)") -> Segments:
    ...
    for table in document.tables():
        ...
        header, footer = get_table_header_footer(
            ...
            header_filter_fn=lambda x: isinstance(x, Paragraph) and table.page_idx == x.page_idx,
            footer_filter_fn=lambda x: isinstance(x, Paragraph) and table.page_idx == x.page_idx,
