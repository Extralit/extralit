...
def run_ab_equivalent_test(pdf_path, num_requests=5, concurrency=5):
    ...
    print(
        f"📋 Command: ab -n {num_requests} -c {concurrency} http://localhost:7860/extract"
    )
    print("=" * 60)
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        futures = [
            executor.submit(test_extraction_endpoint, pdf_path)
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            print(
                f"Request {i:2d}: {status} {result['status_code']} "
    ...
    print(
        f"Total transferred:      {sum(r['content_length'] for r in successful_requests)} bytes"
    )

    if response_times:
        print(
            f"Requests per second:    {len(successful_requests) / total_test_time:.2f} [#/sec] (mean)"
        )
        print(
            f"Time per request:       {statistics.mean(response_times) * 1000:.3f} [ms] (mean)"
        )
        print(
            f"Time per request:       {statistics.mean(response_times) * 1000 / concurrency:.3f} [ms] (mean, across all concurrent requests)"
        )

        print("\nConnection Times (ms):")
        print("              min  mean[+/-sd] median   max")
        print(
            f"Processing:  {min(response_times) * 1000:4.0f} {statistics.mean(response_times) * 1000:4.0f}   {statistics.stdev(response_times) * 1000 if len(response_times) > 1 else 0:4.0f} {statistics.median(response_times) * 1000:6.0f} {max(response_times) * 1000:5.0f}"
        )
...
def run_comprehensive_test():
    ...
    for pdf_path in pdf_files:
        ...
        all_results.append(
            {"filename": Path(pdf_path).name, "pdf_path": pdf_path, **result}
        )
    ...
    for result in all_results:
        ...
        if response_times:
            ...
            print(
                f"   📏 {pdf_info['file_size_mb']:.1f}MB, {pdf_info['page_count']} pages"
            )
            print(f"   ⏱️  ~{avg_time:.1f}s processing time")
            print(f"   📝 {avg_chars:,.0f} characters extracted")
            print(
                f"   ✅ {result['successful_requests']}/{result['successful_requests'] + result['failed_requests']} requests successful"
            )
    ...
    if all_results:
        best_result = min(
            all_results,
            key=lambda x: statistics.mean(x["response_times"])
            if x["response_times"]
            else float("inf"),
        )
        ...
        if best_result["response_times"]:
            ...
            avg_chars = (
                statistics.mean(best_result["markdown_lengths"])
                if best_result["markdown_lengths"]
                else 0
            )
            ...
            print("Performance")
            print(
                f"✅ Processes {pdf_info['file_size_mb']:.1f}MB, {pdf_info['page_count']}-page research papers in ~{avg_time:.0f} seconds"
            )
