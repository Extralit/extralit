...
3. WHEN a PDF filename matches a .bib entry reference THEN the system SHALL associate the PDF with that bibliographic entry
4. WHEN I provide a collection tag THEN the system SHALL add this tag to all imported documents' metadata
5. WHEN documents are processed THEN the system SHALL store the reference as the unique identifier for deduplication
...
4. WHEN duplicate references exist in the .bib file THEN the system SHALL handle them appropriately and warn the user
