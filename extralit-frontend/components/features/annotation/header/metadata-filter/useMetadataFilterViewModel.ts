...
export const useMetadataFilterViewModel = ({ datasetMetadata }: { datasetMetadata: Metadata[] }) => {
  const { getValue } = useFeatureToggle();
  const debounce = useDebounce(getValue("metadata-filter-delay", "integer") ?? 500);
  const metadataFilters = ref<MetadataFilterList>(new MetadataFilterList(datasetMetadata));
