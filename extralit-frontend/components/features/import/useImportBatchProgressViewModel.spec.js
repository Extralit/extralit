...
describe("useImportBatchProgressViewModel", () => {
  ...
  describe("convertToLegacySummary", () => {
    it("correctly converts normalized summary to legacy format", () => {
      const normalizedSummary = {
        ...
        failed: 1,
        errors: [
          { reference: "ref1", message: "Error 1" },
          { reference: "ref2", message: "Error 2" },
        ],
        importId: "test-123",
      };

      const result = viewModel.convertToLegacySummary(normalizedSummary);

      expect(result.totalProcessed).toBe(10);
      expect(result.successfullyAdded).toBe(6);
    ...
    it("handles missing importId", () => {
      ...
      expect(result.importId).toBe(null);
    });
  });

  describe("legacy createSummaryData method", () => {
    it("creates legacy summary data format", () => {
      const errors = [
        { reference: "ref1", message: "Error 1" },
        { reference: "ref2", message: "Error 2" },
      ];

      const result = viewModel.createSummaryData(100, 80, 5, errors);

      expect(result.totalProcessed).toBe(100);
      expect(result.successfullyAdded).toBe(80);
